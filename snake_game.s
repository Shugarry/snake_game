...
.data	#define the state of the game 
	...
	#challenge 4 
	paused: .word 0 
	pause_key: .word 112 #ascii p 
	
	#challenge 7 
	difficulty_msg: .asciz "\nSELECT DIFFICULTY: \n1 Easy level  \n2 Medium level  \n3 Hard level"
	difficulty_level: .word 100 #defualt medium 
	confirmed_msg: .asciz "\nstarting snake game\n" 
...
check_pause_key: 
	...
	#load keyboard crontrol address 
	la t0, input 
	lw t0, 0(t0) #t0 = 0xFFFF00000 
	
	#if the key is pressed 
	lw t1, 0(t0) 
	beqz t1, exit_check #no key pressed exit 
	...
	#check if p 112
	li t1, 112 
	bne t2, t1, exit_check  #not p exit 
	
	#p was pressed 
	la t0, paused 
	lw t1, 0(t0) 
	xori t1, t1, 1  #toggle 0 to 1 or 1 to 0 
	sw t1, 0(t0) #storing new state 
...
main_loop:
	...
	lw t0, 0(t0) 
	lw t1, 0(t0) 
	beqz t1, input_checked #no input, skip 
...
#challenge 7: difficulty selection 
difficulty_select: 
...
pause_loop: 
	...
	la t0, paused 
	lw t1, 0(t0) 
	bnez t1, pause_loop #if still paused, loop back 
	#no longer paused
	j main_loop 
